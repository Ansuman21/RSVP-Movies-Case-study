...
SELECT table_name as Table_Names, table_rows as Total_No_Of_Rows
...
SELECT 
		SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS ID_NullCount, 
		...
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS Date_Published_NullCount,
		...
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS Worlwide_Gross_Income_NullCount,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS Languages_NullCount,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS Production_Company_NullCount
...
-- Type your code below:
 ...
 Round(Sum(avg_rating)/count(total_votes),2) AS actress_avg_rating,
